...
public class MedicsDbHelper extends SQLiteOpenHelper {
    ...
    public void onCreate(SQLiteDatabase db) {
        db.execSQL("CREATE TABLE " + MedicsContract.MedicsEntry.TABLE_NAME +"("
                ...
                + MedicsContract.MedicsEntry.FAV + " TEXT NOT NULL,"
    ...
    public static MedicsDbHelper GetDBHelper(Context context)
    {
        MedicsDbHelper dbHelper = new MedicsDbHelper(context);
    ...
    public boolean updateMedFav(String id)
    {
        SQLiteDatabase db = this.getWritableDatabase();
        db.execSQL(" UPDATE " + MedicsContract.MedicsEntry.TABLE_NAME  + " SET fav = '1' WHERE id = '"+id+"'" );
        return true;
    }

    public MedicsData showMedFav()
    {
        SQLiteDatabase db = this.getWritableDatabase();
        MedicsData md = new MedicsData();
        Cursor cursor = db.rawQuery(" SELECT nameGeneric FROM " + MedicsContract.MedicsEntry.TABLE_NAME  + " WHERE fav = '1' ", null);

        cursor.moveToFirst();

        while (cursor.isAfterLast() == false)
        {
            md.setIndications (cursor.getString(cursor.getColumnIndex("nameGeneric")));
            cursor.moveToNext();
        }
        return md;
    }
    private void mockData(SQLiteDatabase sqLiteDatabase){
        mockLawyer(sqLiteDatabase, new MedicsData("1", "0","antibioticos.jpg","Azitromicina","ZITROMAX, TRI AZIT","AZITROMICINA: Cáp. 250mg, Comp. 500mg, Susp. Oral 1g, Susp. Oral 200mg/5ml, Susp. Oral 250mg/5ml, Susp. Oral 500mg/5ml, vial 500mg.",
        ...
        mockLawyer(sqLiteDatabase, new MedicsData("1","0", "antibioticos.jpg","Claritromicina","KLARICID","CLARITROMICINA: Tab. 250mg y 500mg, Tab. Retard 500mg, Susp. Oral 125 mg/5ml y 250mg/5ml, Vía EV 500mg.",
        ...
        mockLawyer(sqLiteDatabase, new MedicsData("1","0", "antibioticos.jpg",
                "Eritromicina ",
        ...
        mockLawyer(sqLiteDatabase, new MedicsData("2","0", "antibioticos.jpg",
                "Ciprofloxacino",
